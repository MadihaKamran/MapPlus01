...
public class GoogleParser extends XMLParser implements Parser {
        ...
        try {
            ...
            for (int i = 0; i < jsonRoutes.length(); i++) {
                ...
                for (int y = 0; y < numSteps; y++) {
                    ...
                    route.addPoints(decodePolyLine(step.getJSONObject("polyline").getString("points"),y));
                    //Push a copy of the segment to the route
                    route.addSegment(segment.copy());
                    final JSONObject duration = step.getJSONObject("duration");
                    route.addDuration(duration.getInt(VALUE));
                }
    ...
    private List<LatLng> decodePolyLine(final String poly, int stepIndex) {
        int len = poly.length();
        int index = 0;
        ...
        // we only want to return the first one and last one
        if(stepIndex == 0)
        {
            // Only for the first segments, we include both starting point and ending point
            // For all the other ones, we just include the ending points
            // since kth starting point = (k-1)th ending point
            ret.add(decoded.get(0));
        }

        ret.add(decoded.get(decoded.size()-1));
        return ret;
    }
}
